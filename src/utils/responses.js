...
const emptyMentionResponse = `*chittering excitedly* Oh! Oh! <USER>! 🐿️ You called me? I was just... wait, was I organizing my nut collection or debugging code? Both? Anyway! 
...
const helpResponse = `*stops mid-leap between branches* Oh! <USER> wants to know what I can do! 🐿️
...
const statusResponse = (aiStatus, uptimeString) => `*scurries around checking things with tiny clipboard* <USER> 📋🐿️
...
const infoResponse = (aiStatus, kbList) => `*adjusts tiny glasses and shuffles through acorn notes* <USER> 📚🐿️
...
const getRandomGreeting = (userId) => {
  const greeting = greetings[Math.floor(Math.random() * greetings.length)];
  return greeting.replace('<USER>', `<@${userId}>`);
};

const getRandomThankYou = (userId) => {
  const thanks = thankYouResponses[Math.floor(Math.random() * thankYouResponses.length)];
  return thanks.replace('<USER>', `<@${userId}>`);
};

const getRandomThinkingPrefix = (userId) => {
  const prefix = thinkingPrefixes[Math.floor(Math.random() * thinkingPrefixes.length)];
  return prefix.replace('<USER>', `<@${userId}>`);
};

const getEmptyMentionResponse = (userId) => {
  return emptyMentionResponse.replace('<USER>', `<@${userId}>`);
};

const getHelpResponse = (userId) => {
  return helpResponse.replace('<USER>', `<@${userId}>`);
};

const getStatusResponse = (userId, aiStatus, uptimeString) => {
  return statusResponse(aiStatus, uptimeString).replace('<USER>', `<@${userId}>`);
};

const getInfoResponse = (userId, aiStatus, kbList) => {
  return infoResponse(aiStatus, kbList).replace('<USER>', `<@${userId}>`);
};

// AI Handler responses
const askEmptyResponse = `*tilts head* You said "ask:" but then... *looks around confused* ...where's the question? Try: \`ask: What's the best way to store acorns?\` 🐿️`;

const askThinkingResponse = `*scurries up thinking tree* 🌳 Let me check my nut collection first, then think...`;


const kbNotFoundResponse = (kbIndex, totalKbs) => 
...
const kbEmptyQuestionResponse = (kbIndex) => 
